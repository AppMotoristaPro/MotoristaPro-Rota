...
CURRENT_BACKUP_PATH = os.path.join(BACKUP_DIR, f"update_v23_{TIMESTAMP}")
...
const DB_KEY = 'mp_db_v61_distance_matrix';
...
// --- ALGORITMO V23: DISTANCE MATRIX GREEDY (Menor Distância Real) ---
const optimizeByDrivingDistance = async (allLocations, startPos, updateProgress) => {
    ...
    while (unvisited.length > 0) {
        ...
        // Se só sobrou 1, finaliza
        if (unvisited.length === 1) {
            finalChain.push(unvisited[0]);
            break;
        }

        // 1. Pré-filtro Geográfico (Economia de API)
        // Ordenamos por linha reta apenas para selecionar QUEM vamos medir com precisão.
        // O Google limita a matrix em 25 destinos por chamada.
        unvisited.sort((a, b) => {
        ...
        // Pega os 25 candidatos mais prováveis
        const candidates = unvisited.slice(0, 25);
        
        // 2. A Mágica: Distance Matrix API
        // Pergunta a distância real de RODAGEM (não tempo) para os candidatos
        try {
            ...
            // 3. Seleção do Vencedor por METROS
            ...
            for (let i = 0; i < results.length; i++) {
                ...
                if (element.status === 'OK') {
                    // Aqui está o segredo: usamos distance.value (metros), ignoramos duration
                    if (element.distance.value < minMeters) {
                        minMeters = element.distance.value;
                        bestCandidateIdx = i;
                    }
                }
            }

            // Fallback: Se a API falhar em calcular rotas (ex: ilha sem ponte), pega o 1º da lista geométrica
            if (bestCandidateIdx === -1) bestCandidateIdx = 0;

            const winner = candidates[bestCandidateIdx];
            ...
            // Delay curto para evitar rate limit (Matrix consome mais quota)
            await new Promise(r => setTimeout(r, 350));

        } catch (e) {
            console.warn("Matrix Error, fallback geo", e);
...
export default function App() {
  ...
  const optimizeRoute = async () => {
      ...
      try {
          ...
          // NOVA OTIMIZAÇÃO: MATRIX REAL DISTANCE
          const optimizedLocs = await optimizeByDrivingDistance(locations, pos, (current, total) => {
          ...
          setRoutes(updated);
          showToast("Otimização por Menor Distância Concluída!");
                      ...
                      >
                          {isOptimizing ? (
                              <div className="flex items-center gap-2">
                                ...
                                <span>{optimizeProgress ? `Dist. Real ${optimizeProgress}` : 'Calculando...'}</span>
                              </div>
                          ) : (
                              <div className="flex items-center gap-2">
...
def main():
    print(f"--- Iniciando V23 (Distance Matrix Greedy) {TIMESTAMP} ---")
    write_files()
    
    print("--- Git Push ---")
    subprocess.run("git add .", shell=True)
    subprocess.run(f'git commit -m "Update V23: Distance Matrix Optimization - {TIMESTAMP}"', shell=True)
