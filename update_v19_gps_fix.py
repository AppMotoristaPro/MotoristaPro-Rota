...
# 2. APP.JSX (Correção da Lógica de Otimização e GPS)
...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Tenta pegar GPS silenciosamente no inicio
  ...
  const getCurrentLocation = async (force = false) => {
      try {
          // Se forçado, pede permissão. Se não, só tenta se já tiver.
          if (force) {
              const perm = await Geolocation.requestPermissions();
              if (perm.location !== 'granted') throw new Error('Permissão negada');
          }
  ...
  // --- OTIMIZAÇÃO COM GPS REAL ---
  const optimizeRoute = async () => {
      setIsOptimizing(true);
      
      // 1. Tenta pegar a localização ATUAL (Força permissão se necessário)
      let currentPos = userPos;
      if (!currentPos) {
          currentPos = await getCurrentLocation(true);
      }

      if (!currentPos) {
          setIsOptimizing(false);
          alert("Não conseguimos sua localização. Verifique se o GPS está ativado e tente novamente.");
          return;
      }

      const rIdx = routes.findIndex(r => r.id === activeRouteId);
      if (rIdx === -1) { setIsOptimizing(false); return; }
      ...
      // Começa da posição ATUAL do usuário
          ...
          <div className="bg-white px-5 py-4 shadow-sm z-20 sticky top-0">
              ...
              <div className="flex gap-3">
                  <button 
                      ...
                      className={`flex-1 py-3 rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition ${!activeRoute.optimized ? 'btn-highlight animate-pulse' : 'btn-secondary'}`}
                  >
                      {isOptimizing ? <Loader2 className="animate-spin" size={18}/> : <Sliders size={18}/>} 
                      {isOptimizing ? 'Localizando...' : (activeRoute.optimized ? 'Reotimizar' : 'Otimizar Rota')}
                  </button>
                  ...
                  {nextGroup && (
                      <button 
                          ...
                          className={`flex-[1.5] py-3 rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition ${activeRoute.optimized ? 'btn-highlight shadow-lg' : 'bg-slate-100 text-slate-300 cursor-not-allowed'}`}
                      >
...
def main():
    print(f"🚀 ATUALIZAÇÃO V19 (GPS FIX) - {APP_NAME}")
    ...
    subprocess.run('git commit -m "fix: V19 Force Active GPS on Optimization"', shell=True)
