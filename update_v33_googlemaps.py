...
# 1. PACKAGE.JSON (Adicionando a lib do Google Maps e removendo Leaflet)
...
# 2. CSS (Limpando Leaflet e ajustando container)
...
# 3. APP.JSX (Google Maps Integration)
files_content['src/App.jsx'] = f'''import React, {{ useState, useEffect, useMemo, useRef, useCallback }} from 'react';
import {{ 
...
const mapOptions = {{
    disableDefaultUI: true, // Remove botões padrão do Google para visual limpo
    zoomControl: false,
    streetViewControl: false,
    mapTypeControl: false,
    fullscreenControl: false,
    clickableIcons: false // Evita clicar em lojas do mapa
}};
...
const groupStopsByStopName = (stops) => {{
    ...
    stops.forEach(stop => {{
        ...
        if (!groups[key]) {{
            groups[key] = {{
                id: key, lat: stop.lat, lng: stop.lng,
                mainName: rawName, mainAddress: stop.address,
                                  ...
                                  <div className="bg-slate-50 border-t border-slate-100 px-4 py-2 space-y-3">
                                      {{group.items.map((item, subIdx) => (
                                          <div key={{item.id}} className="flex flex-col py-2 border-b border-slate-200 last:border-0">
                                              <div className="mb-2"><span className="text-[10px] font-bold text-blue-500 block">PARADA #{{subIdx + 1}}</span><span className="text-sm font-bold text-slate-700 block">{{item.address}}</span></div>
...
def main():
    print(f"🚀 ATUALIZAÇÃO V33 (GOOGLE MAPS) - {APP_NAME}")
    ...
    subprocess.run("npm install @react-google-maps/api", shell=True)
    subprocess.run("npm uninstall leaflet react-leaflet", shell=True)
    subprocess.run("npx cap sync", shell=True)

    print("\n☁️ Enviando para GitHub...")
    subprocess.run("git add .", shell=True)
    subprocess.run('git commit -m "feat: V33 Google Maps JS API + UI Polish"', shell=True)
