...
CURRENT_BACKUP_PATH = os.path.join(BACKUP_DIR, f"update_v22_{TIMESTAMP}")
...
const DB_KEY = 'mp_db_v60_street_sweeper';
...
const getStreetName = (address) => {
    // Normalização agressiva para agrupar ruas
    const raw = safeStr(address).toLowerCase();
    // Pega o que vem antes da primeira vírgula ou número
    const parts = raw.split(/,|\d+/);
    let street = parts[0].trim();
    
    // Remove prefixos comuns para garantir agrupamento (Rua das Flores = Flores)
    street = street.replace(/^(rua|av|avenida|travessa|alameda|rodovia|estrada|r\.)\s+/, '');
    
    return street || "desconhecido";
};

// Extrai número para ordenação interna
...
const calculateTotalMetrics = (stops) => {
    ...
    const realKm = totalKm * 1.3; // Fator de correção
    const avgSpeed = 25; 
...
// --- ALGORITMO "VASSOURA DE RUA" (STREET SWEEPER) ---
const optimizeStreetSweeper = async (allLocations, startPos, updateProgress) => {
    // 1. Agrupamento por Rua (Clusterização Rígida)
    ...
    let clustersPool = Object.values(streetClusters);
    let finalRoute = [];
    let currentPos = startPos;
    const totalClusters = clustersPool.length;

    // 2. Loop de Seleção de Rua (Macro Otimização)
    while (clustersPool.length > 0) {
        
        // Encontra a rua mais próxima da posição atual
        // A distância da rua é definida pela distância do ponto mais próximo nela
        let bestClusterIdx = -1;
        let minClusterDist = Infinity;
        let bestEntrancePoint = null;

        for (let i = 0; i < clustersPool.length; i++) {
            const cluster = clustersPool[i];
            
            // Verifica todos os pontos da rua para ver qual está mais perto
            for (let p of cluster) {
                const d = Math.pow(p.lat - currentPos.lat, 2) + Math.pow(p.lng - currentPos.lng, 2);
                if (d < minClusterDist) {
                    minClusterDist = d;
                    bestClusterIdx = i;
                    bestEntrancePoint = p; // Ponto de entrada ideal nessa rua
                }
            }
        }

        const selectedCluster = clustersPool[bestClusterIdx];
        
        // 3. Ordenação Interna da Rua (Micro Otimização)
        // Decide a direção: Crescente ou Decrescente baseado no ponto de entrada
        const entranceNum = getHouseNumber(bestEntrancePoint.address);
        
        // Ordena a rua inteira por número
        selectedCluster.sort((a, b) => getHouseNumber(a.address) - getHouseNumber(b.address));
        
        // Verifica se é melhor inverter (se entrou pelo final/número alto)
        const firstNum = getHouseNumber(selectedCluster[0].address);
        const lastNum = getHouseNumber(selectedCluster[selectedCluster.length - 1].address);
        
        // Se o ponto de entrada for mais próximo do maior número do que do menor, inverte
        if (Math.abs(entranceNum - lastNum) < Math.abs(entranceNum - firstNum)) {
            selectedCluster.reverse();
        }

        // Adiciona a rua inteira ordenada à rota final
        finalRoute.push(...selectedCluster);
        
        // A nova posição é o último ponto desta rua
        currentPos = selectedCluster[selectedCluster.length - 1];
        
        // Remove do pool
        clustersPool.splice(bestClusterIdx, 1);

        if (updateProgress) updateProgress(finalRoute.length, allLocations.length);
        
        // Pequeno delay visual para UX (opcional, já que é local e rápido)
...
export default function App() {
  ...
  const optimizeRoute = async () => {
      ...
      // Mapeia para otimização usando apenas um representativo
      const locations = groups.map(g => ({ ...g.items[0] })); 
      
      try {
          setOptimizeProgress("0%");
          // NOVA OTIMIZAÇÃO V22: VASSOURA DE RUA (STREET SWEEPER)
          // Isso é muito mais rápido e não gasta API do Google para ordenar
          const optimizedLocs = await optimizeStreetSweeper(locations, pos, (current, total) => {
          ...
          setRoutes(updated);
          showToast("Otimização por Rua Concluída!");
  ...
  // Google Directions Rendering (Apenas visual, não afeta a ordem)
  useEffect(() => {
                      ...
                      >
                          {isOptimizing ? (
                              <div className="flex items-center gap-2">
                                ...
                                <span>{optimizeProgress ? `Org. Rua ${optimizeProgress}` : 'Calculando...'}</span>
                              </div>
                          ) : (
                              <div className="flex items-center gap-2">
...
def main():
    print(f"--- Iniciando V22 (Vassoura de Rua) {TIMESTAMP} ---")
    write_files()
    
    print("--- Git Push ---")
    subprocess.run("git add .", shell=True)
    subprocess.run(f'git commit -m "Update V22: Street Sweeper Logic - {TIMESTAMP}"', shell=True)
