...
# 1. PACKAGE.JSON (Adicionando Leaflet)
...
/* Indicadores de Status do Grupo */
.status-pending { border-left: 4px solid #3B82F6; } /* Azul */
.status-success { border-left: 4px solid #10B981; background-color: #F0FDF4; } /* Verde */
.status-failed { border-left: 4px solid #EF4444; background-color: #FEF2F2; } /* Vermelho */
.status-partial { border-left: 4px solid #F59E0B; background-color: #FFFBEB; } /* Laranja */
...
.toast-enter-active { transform: translateY(0); opacity: 1; transition: all 0.3s; }
...
.pin-current { background-color: #0F172A; transform: scale(1.2); border: 3px solid white; z-index: 1000 !important; }
'''

# 3. APP.JSX (Com Modo Mapa)
...
const createCustomIcon = (status, isCurrent) => {
    ...
    return L.divIcon({
        ...
        iconSize: isCurrent ? [30, 30] : [20, 20],
        iconAnchor: isCurrent ? [15, 15] : [10, 10]
    });
};
...
const groupStopsByStopName = (stops) => {
    ...
    stops.forEach(stop => {
        ...
        if (!seenKeys.has(key)) {
            ...
            else if (success + failed === total) group.status = 'partial';
...
export default function App() {
  ...
  const [showMap, setShowMap] = useState(false); // Toggle entre Lista e Mapa
          ...
          <div className="bg-white px-5 py-4 shadow-sm z-20 sticky top-0">
              <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="flex gap-2">
                      <button onClick={() => setShowMap(!showMap)} className="p-2 text-slate-600 bg-slate-100 rounded-full">
          ...
          {showMap ? (
              <div className="flex-1 relative">
                  <MapContainer center={[-23.55, -46.63]} zoom={13} style={{ height: '100%', width: '100%' }}>
                      ...
                      {nextGroup && <MapRecenter center={[nextGroup.lat, nextGroup.lng]} />}
                      {groupedStops.map((group, idx) => (
                          <Marker 
                              key={group.id} 
                              position={[group.lat, group.lng]}
                              icon={createCustomIcon(group.status, nextGroup && group.id === nextGroup.id)}
                          >
                              <Popup>
                                  <div className="text-center">
                                      <strong className="block text-sm">{group.mainName}</strong>
                                      <span className="text-xs text-gray-500">{group.items.length} pacotes</span>
                                      <button onClick={() => openNav(group.lat, group.lng)} className="mt-2 w-full bg-blue-600 text-white text-xs py-1 rounded">Navegar</button>
                                  </div>
                              </Popup>
                          </Marker>
                      ))}
              ...
              <div className="flex-1 overflow-y-auto px-5 pt-4 pb-safe space-y-3">
                  ...
                  {filteredGroups.map((group, idx) => {
                      ...
                      if (group.status === 'success') statusClass = "status-success";
                      if (group.status === 'failed') statusClass = "status-failed";
                      if (group.status === 'partial') statusClass = "status-partial";
...
def main():
    print(f"🚀 ATUALIZAÇÃO V23 (MAP VIEW) - {APP_NAME}")
    ...
    subprocess.run('git commit -m "feat: V23 Map View Feature"', shell=True)
