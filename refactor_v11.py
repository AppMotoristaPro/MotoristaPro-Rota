...
CURRENT_BACKUP_PATH = os.path.join(BACKUP_DIR, f"update_v11_opt_{TIMESTAMP}")

# CHAVE API (Mantida hardcoded como solicitado anteriormente)
API_KEY = "AIzaSyB8bI2MpTKfQHBTZxyPphB18TPlZ4b3ndU"

# --- NOVO CONTEÚDO DO APP.JSX ---
APP_JSX_CONTENT = f"""import React, {{ useState, useEffect, useMemo }} from 'react';
import {{ 
...
const DB_KEY = 'mp_db_v52_global_opt';
...
// --- NOVA LÓGICA DE OTIMIZAÇÃO (GLOBAL + BATCH) ---
const optimizeGlobalBatches = async (allLocations, startPos) => {{
    // 1. ORDENAÇÃO GLOBAL PRÉVIA (Vizinho Mais Próximo Puro)
    // Isso cria uma "espinha dorsal" lógica para a rota inteira antes de enviar pro Google
    let rawList = [...allLocations];
    let globalOrdered = [];
    ...
    while (rawList.length > 0) {{
        ...
        for(let i=0; i<rawList.length; i++) {{
            const d = Math.pow(rawList[i].lat - currentPointer.lat, 2) + Math.pow(rawList[i].lng - currentPointer.lng, 2);
            if (d < minDist) {{ minDist = d; nearestIdx = i; }}
        }}
        
        const nextStop = rawList[nearestIdx];
        globalOrdered.push(nextStop);
        currentPointer = nextStop;
        rawList.splice(nearestIdx, 1);
    }}

    // 2. DIVISÃO EM LOTES E OTIMIZAÇÃO FINA
    const service = new window.google.maps.DirectionsService();
    const BATCH_SIZE = 23; // Google Limit (23 waypoints + 1 dest)
    let finalOptimizedList = [];
    
    // Processa em lotes seguindo a ordem global pré-calculada
    for (let i = 0; i < globalOrdered.length; i += BATCH_SIZE) {{
        const batch = globalOrdered.slice(i, i + BATCH_SIZE);
        
        // Ponto de partida deste lote (é o último ponto do lote anterior ou a posição do usuário)
        const batchOrigin = (finalOptimizedList.length > 0) 
            ? finalOptimizedList[finalOptimizedList.length - 1] 
            : startPos;
            
        // Se o lote for muito pequeno (1 item), não precisa mandar pro Google reordenar
        if (batch.length <= 1) {{
            finalOptimizedList.push(...batch);
            continue;
        }}

        // O destino é o último ponto deste lote pré-ordenado
        const batchDest = batch[batch.length - 1];
        // Os intermediários são todos menos o último
        ...
        try {{
            const res = await new Promise((resolve, reject) => {{
                service.route({{
                    ...
                    waypoints: waypoints,
                    optimizeWaypoints: true, // Google reorganiza internamente este lote
                    travelMode: 'DRIVING'
                }}, (result, status) => {{
            ...
            // Reaplica a ordem do Google ao lote
            const order = res.routes[0].waypoint_order;
            // O array 'batch' tem (waypoints + destination). O waypoint_order refere-se apenas aos waypoints.
            // Precisamos reconstruir: [reordered_waypoints, destination]
            
            // Itens intermediários reordenados
            const reorderedWaypoints = order.map(idx => batch[idx]); 
            
            // Adiciona na lista final: Intermediários + Destino
            ...
            console.warn("Google API Batch Fail, using pre-sort:", e);
            finalOptimizedList.push(...batch); // Fallback: usa a ordem pré-calculada
        }}
    }}
...
export default function App() {{
  ...
  const optimizeRoute = async () => {{
      ...
      try {{
          ...
          const optimizedLocs = await optimizeGlobalBatches(locations, pos);
          ...
          setRoutes(updated);
          showToast("Otimizado Globalmente!");
...
def main():
    print(f"--- Iniciando V11 {TIMESTAMP} ---")
    write_files()
    
    subprocess.run("git add .", shell=True)
    subprocess.run(f'git commit -m "Update V11: Otimização Global + Lotes - {TIMESTAMP}"', shell=True)
