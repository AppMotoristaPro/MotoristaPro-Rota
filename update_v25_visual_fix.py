...
# 1. CSS (Reforçando estilos e Leaflet)
...
/* Garante que o mapa ocupe altura correta */
.leaflet-container {
  width: 100%;
  height: 100%;
  z-index: 0;
  background-color: #e5e7eb; /* Cinza enquanto carrega */
}
...
.border-l-status-success { border-left: 6px solid #10B981; background-color: #F0FDF4; }
.border-l-status-failed { border-left: 6px solid #EF4444; background-color: #FEF2F2; }
...
/* Botões de Fluxo (Contorno e Destaque) */
.btn-primary-outline {
...
# 2. APP.JSX (Correção de Lógica e Renderização)
...
const DB_KEY = 'mp_db_v25_visualfix';

// --- COMPONENTE PARA CORRIGIR MAPA BRANCO ---
function MapFixer({ active }) {
  const map = useMap();
  useEffect(() => {
    if (active) {
      setTimeout(() => { map.invalidateSize(); }, 200); // Força redesenho
    }
  }, [active, map]);
  return null;
}
...
// --- HELPERS ---
const createCustomIcon = (status, isCurrent) => {
    let color = '#3B82F6'; // Azul
    if (status === 'success') color = '#10B981'; // Verde
    if (status === 'failed') color = '#EF4444'; // Vermelho
    if (status === 'partial') color = '#F59E0B'; // Amarelo
    if (isCurrent) color = '#0F172A'; // Preto (Atual)

    return L.divIcon({
        className: 'custom-pin-container',
        html: `<div style="background-color: ${color}; width: 100%; height: 100%; border-radius: 50%; border: 2px solid white; display: flex; align-items: center; justify-content: center; color: white; font-weight: bold; box-shadow: 0 2px 5px rgba(0,0,0,0.3); font-size: ${isCurrent ? '14px' : '10px'}">${status === 'success' ? '✓' : ''}</div>`,
        iconSize: isCurrent ? [36, 36] : [24, 24],
        iconAnchor: isCurrent ? [18, 18] : [12, 12]
    });
};
...
const groupStopsByStopName = (stops) => {
    ...
    stops.forEach(stop => {
        ...
        if (!seenKeys.has(key)) {
            ...
            else if (success + failed === total) group.status = 'partial'; // Finalizado misto
            else if (success + failed > 0) group.status = 'partial'; // Em andamento misto
...
export default function App() {
  ...
  const autoReoptimize = (currentStops, currentPos) => {
      if (!currentPos) return currentStops;
      let pending = currentStops.filter(s => s.status === 'pending');
      let done = currentStops.filter(s => s.status !== 'pending');
      if (pending.length === 0) return currentStops;
      ...
      return [...done, ...optimized];
  };

  const optimizeRouteManual = async () => {
      ...
      if (!currentPos) {
          ...
          alert("Ative o GPS para otimizar.");
          return;
      }
      ...
      const newStops = autoReoptimize(routes[rIdx].stops, currentPos);
      
      const updatedRoutes = [...routes];
      updatedRoutes[rIdx] = { ...updatedRoutes[rIdx], stops: newStops, optimized: true };
      
      setRoutes(updatedRoutes);
      setIsOptimizing(false);
      showToast("Rota Reorganizada!", "success");
  };
  ...
  const setStatus = (stopId, status) => {
      ...
      if (stopIndex !== -1) {
          ...
          if (route.optimized && userPos) {
              route.stops = autoReoptimize(route.stops, userPos);
          }
          setRoutes(updatedRoutes);
          if (status === 'success') showToast("Pacote Entregue!", "success");
          else showToast("Ocorrência Registrada", "error");
      }
  };
  ...
  }, [activeRoute, routes]); // Importante: Depender de 'routes' para atualizar quando setStatus mudar
  ...
  const filteredGroups = useMemo(() => {
      ...
      return groupedStops.filter(g => 
          ...
          g.items.some(i => i.recipient.toLowerCase().includes(lower))
      );
          ...
          {showMap ? (
              <div className="flex-1 relative bg-slate-100">
                  <MapContainer center={userPos || [-23.55, -46.63]} zoom={13} style={{ height: '100%', width: '100%' }}>
                      ...
                      <MapFixer active={showMap} />
                      {nextGroup && <Marker position={[nextGroup.lat, nextGroup.lng]} icon={createCustomIcon('pending', true)} />}
                      {groupedStops.map((group) => {
                          if(nextGroup && group.id === nextGroup.id) return null;
                          return <Marker key={group.id} position={[group.lat, group.lng]} icon={createCustomIcon(group.status, false)} />
                      })}
                  </MapContainer>
              </div>
          ) : (
              <div className="flex-1 overflow-y-auto px-5 pt-4 pb-safe space-y-3">
                  {!searchQuery && nextGroup && activeRoute.optimized && (
                      <div className="modern-card p-6 border-l-8 border-slate-900 bg-white relative mb-6 shadow-md transition-all duration-500">
                              ...
                              {nextGroup.items.map(item => {
                                  ...
                                  return (
                                      <div key={item.id} className="flex flex-col bg-slate-50 p-3 rounded-lg border border-slate-100">
                                          ...
                                          <div className="flex gap-2 w-full"><button onClick={() => setStatus(item.id, 'failed')} className="flex-1 btn-action-lg bg-white border-2 border-red-100 text-red-600 rounded-xl hover:bg-red-50"><AlertTriangle size={20} className="mb-1"/> Falha</button><button onClick={() => setStatus(item.id, 'success')} className="flex-1 btn-action-lg bg-green-600 text-white rounded-xl shadow-lg active:scale-95"><Check size={24} className="mb-1"/> ENTREGUE</button></div>
                                      </div>
                                  )
                              })}
                          </div>
...
def main():
    print(f"🚀 ATUALIZAÇÃO V25 (VISUAL FIX) - {APP_NAME}")
    ...
    subprocess.run('git commit -m "fix: V25 CSS Classes Fix, Map Resize Trigger & Button Outlines"', shell=True)
