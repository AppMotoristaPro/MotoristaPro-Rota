...
CURRENT_BACKUP_PATH = os.path.join(BACKUP_DIR, f"update_v15_{TIMESTAMP}")

# CHAVE API
API_KEY_VALUE = "AIzaSyB8bI2MpTKfQHBTZxyPphB18TPlZ4b3ndU"

# --- COMPONENTE ROUTELIST (COM BOTÃO DE EDITAR) ---
...
# --- COMPONENTE APP (LÓGICA NOVA DE OTIMIZAÇÃO POR RUA) ---
...
const DB_KEY = 'mp_db_v54_smart_opt';
...
const getStreetName = (address) => {
    // Normaliza para extrair o nome da rua (remove números e tipos comuns)
    const raw = safeStr(address).toLowerCase();
    // Ex: "Rua das Flores, 123" -> "rua das flores"
    // Remove numero se tiver vírgula
...
// --- NOVA OTIMIZAÇÃO: STREET LOCKING ---
const optimizeSmartStreets = async (allStops, startPos) => {
    // 1. Agrupa por Nome de Rua (Street Locking)
    const streetMap = {};
    ...
    // Calcula o "centro" de cada rua (média das coordenadas)
    const streetCenters = [];
    Object.keys(streetMap).forEach(key => {
        const items = streetMap[key];
    ...
    // 2. Ordena as RUAS (não os pacotes) usando Vizinho Mais Próximo
    let sortedStreets = [];
    ...
    while (streetsPool.length > 0) {
        ...
        // Atualiza o ponteiro para o fim dessa rua (ou centro dela)
        currentPointer = nextStreet; 
        streetsPool.splice(nearestIdx, 1);
    }

    // 3. Achata a lista: Converte lista de ruas de volta para lista de paradas
    // Isso garante que todas as paradas da Rua A fiquem juntas antes de ir para Rua B
    let flatList = [];
    sortedStreets.forEach(street => {
        flatList.push(...street.items);
    });

    // 4. Refino Google em Lotes (apenas para corrigir mão de direção, mantendo o bloco)
    const service = new window.google.maps.DirectionsService();
    const BATCH_SIZE = 23; 
    let finalOptimizedList = [];
    
    // Processa a lista já estruturada por ruas
    for (let i = 0; i < flatList.length; i += BATCH_SIZE) {
        const batch = flatList.slice(i, i + BATCH_SIZE);
        const batchOrigin = (finalOptimizedList.length > 0) ? finalOptimizedList[finalOptimizedList.length - 1] : startPos;
        
        if (batch.length <= 1) {
            finalOptimizedList.push(...batch);
            continue;
        }
        ...
        try {
            ...
            const reorderedWaypoints = order.map(idx => batch[idx]); 
            finalOptimizedList.push(...reorderedWaypoints);
            finalOptimizedList.push(batchDest);
            await new Promise(r => setTimeout(r, 600));

        } catch (e) {
            finalOptimizedList.push(...batch); // Fallback
        }
    }
...
export default function App() {
  ...
  const updateAddress = async (stopId, newAddress) => {
      ...
      if (stopIndex !== -1) {
          // Atualiza texto visualmente primeiro
          updatedRoutes[rIdx].stops[stopIndex].address = newAddress;
          setRoutes(updatedRoutes);
          
          // Tenta Geocodificar para atualizar o pino no mapa
          try {
              ...
              if (data.status === 'OK' && data.results.length > 0) {
                  ...
                  setRoutes(updatedRoutes); // Re-renderiza mapa
  ...
  const optimizeRoute = async () => {
      ...
      try {
          // NOVA OTIMIZAÇÃO: STREET LOCKING
          const optimizedStops = await optimizeSmartStreets(pending, pos);
          
          const updated = [...routes];
          updated[rIdx] = { ...updated[rIdx], stops: [...done, ...optimizedStops], optimized: true };
          setRoutes(updated);
          showToast("Rota Inteligente Criada!");
...
def main():
    print(f"--- Iniciando V15 {TIMESTAMP} ---")
    write_files()
    
    print("--- Git Push ---")
    subprocess.run("git add .", shell=True)
    subprocess.run(f'git commit -m "Update V15: Otimização por Rua + Edit Endereço - {TIMESTAMP}"', shell=True)
