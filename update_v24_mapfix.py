...
# 1. CSS (Fix do Mapa e Cores de Status)
...
/* Status Colors (Borda Esquerda + Fundo Sutil) */
.status-pending { border-left: 5px solid #3B82F6; }
.status-success { border-left: 5px solid #10B981; background-color: #F0FDF4; }
.status-failed { border-left: 5px solid #EF4444; background-color: #FEF2F2; }
.status-partial { border-left: 5px solid #F59E0B; background-color: #FFFBEB; }
...
.pin-success { background-color: #10B981; }
.pin-failed { background-color: #EF4444; }
.pin-partial { background-color: #F59E0B; }
.pin-current { 
...
# 2. APP.JSX (Lógica Corrigida de Mapa e Reotimização)
...
const DB_KEY = 'mp_db_v24_mapfix';

// --- MAP FIX COMPONENT ---
// Isso resolve o problema do mapa ficar cinza/branco ao mudar de aba
function MapRevalidater() {
  const map = useMap();
  useEffect(() => {
...
const createCustomIcon = (status, isCurrent) => {
    ...
    else if (status === 'partial') className += ' pin-partial';
    else if (isCurrent) className += ' pin-current';

    return L.divIcon({
        className: className,
        html: `<span>${status === 'success' ? '✓' : ''}</span>`,
        iconSize: isCurrent ? [30, 30] : [24, 24],
        iconAnchor: isCurrent ? [15, 15] : [12, 12]
    });
};
...
const groupStopsByStopName = (stops) => {
    ...
    stops.forEach(stop => {
        ...
        if (!seenKeys.has(key)) {
            ...
            else if (success + failed > 0 && success + failed < total) group.status = 'partial'; // Misto pendente
            else if (success + failed === total) group.status = 'partial'; // Misto finalizado (mas vou usar partial para cor amarela)
            else group.status = 'pending';

            // Se tudo finalizado misto (sucesso + falha), mantem partial (amarelo)
            // Se tudo sucesso -> verde
            // Se tudo falha -> vermelho
...
export default function App() {
  ...
  // --- LÓGICA DE REOTIMIZAÇÃO SILENCIOSA ---
  ...
  // Otimização Manual (Com loading e alertas)
  ...
  // Atualizar Status e Reotimizar Automaticamente
  const setStatus = (stopId, status) => {
      ...
      if (stopIndex !== -1) {
          ...
          // 2. Se a rota já era otimizada, reotimiza os pendentes restantes
  ...
  }, [activeRoute]); // Recalcula quando stops mudam (reordenação/status)
  ...
  return (
      <div className="flex flex-col h-screen bg-slate-50">
          {toast && (
              <div className={`fixed top-4 left-4 right-4 p-4 rounded-xl shadow-2xl z-50 text-white text-center font-bold text-sm toast-message ${toast.type === 'success' ? 'bg-green-600' : 'bg-red-600'}`}>
                  {toast.msg}
              </div>
          )}
          ...
          <div className="bg-white px-5 py-4 shadow-sm z-20 sticky top-0">
              ...
              {!searchQuery && !showMap && (
                  <div className="flex gap-3">
                      <button onClick={optimizeRouteManual} disabled={isOptimizing} className={`flex-1 py-3 rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition ${!activeRoute.optimized ? 'btn-highlight animate-pulse' : 'btn-secondary'}`}>
          ...
          {showMap ? (
              ...
              <div className="flex-1 overflow-y-auto px-5 pt-4 pb-safe space-y-3">
                  ...
                  {filteredGroups.map((group) => {
                      ...
                      return (
                          <div key={group.id} className={`modern-card overflow-hidden ${statusClass} ${group.status !== 'pending' && !searchQuery ? 'opacity-50 grayscale' : ''}`}>
...
def main():
    print(f"🚀 ATUALIZAÇÃO V24 (MAP & FLOW FIX) - {APP_NAME}")
    ...
    subprocess.run('git commit -m "fix: V24 Map Render Fix and Auto Reoptimize Logic"', shell=True)
