...
CURRENT_BACKUP_PATH = os.path.join(BACKUP_DIR, f"update_v26_{TIMESTAMP}")
...
const DB_KEY = 'mp_db_v64_street_matrix';
...
const getStreetName = (address) => {
    ...
    // Remove números para pegar só o nome da rua
    const parts = raw.split(/,|\d+/);
    let street = parts[0].trim();
    // Normaliza nomes comuns
    street = street.replace(/^(rua|av|avenida|travessa|alameda|rodovia|estrada|r\.|al\.)\s+/, '');
    return street || "outros";
};
...
// --- ALGORITMO V26: STREET-LEVEL MATRIX ---
// Otimiza a ordem das RUAS usando a Matrix do Google
const optimizeStreetMatrix = async (allLocations, startPos, updateProgress) => {
    
    // 1. Agrupa tudo por Rua
    const streetClusters = {};
    allLocations.forEach(loc => {
        const street = getStreetName(loc.address);
        if (!streetClusters[street]) streetClusters[street] = [];
        streetClusters[street].push(loc);
    });

    // Converte para array de objetos "Rua"
    let pendingStreets = Object.keys(streetClusters).map(key => {
        // Ordena internamente por número (base)
        const items = streetClusters[key].sort((a, b) => getHouseNumber(a.address) - getHouseNumber(b.address));
        return {
            name: key,
            items: items,
            // Ponto representativo (o meio da rua) para cálculo de distância macro
            center: items[Math.floor(items.length / 2)] 
        };
    });
    ...
    const totalStreets = pendingStreets.length;
    let processedCount = 0;

    // 2. Loop: Escolhe a próxima rua mais próxima (em tempo real)
    while (pendingStreets.length > 0) {
        
        // Se só tem 1, vai nela
        if (pendingStreets.length === 1) {
            const lastStreet = pendingStreets[0];
            finalRoute.push(...lastStreet.items);
            break;
        }

        // PRÉ-FILTRO: Pega as 10 ruas mais próximas geograficamente para medir com precisão
        pendingStreets.sort((a, b) => {
            const dA = Math.pow(a.center.lat - currentOrigin.lat, 2) + Math.pow(a.center.lng - currentOrigin.lng, 2);
            const dB = Math.pow(b.center.lat - currentOrigin.lat, 2) + Math.pow(b.center.lng - currentOrigin.lng, 2);
            return dA - dB;
        });

        const candidates = pendingStreets.slice(0, 10); // Mede as Top 10 ruas

        try {
            // Pergunta ao Google: Qual rua está mais perto de onde eu estou?
            const response = await new Promise((resolve, reject) => {
                service.getDistanceMatrix({
                    origins: [currentOrigin],
                    destinations: candidates.map(c => ({ lat: c.center.lat, lng: c.center.lng })),
                    travelMode: 'DRIVING',
            ...
            elements.forEach((el, idx) => {
                if (el.status === 'OK' && el.distance.value < minVal) {
                    minVal = el.distance.value; // Menor distância em metros
                    bestIdx = idx;
                }
            });

            const winnerStreet = candidates[bestIdx];

            // DECISÃO DE SENTIDO DA RUA (Entrada pelo menor ou maior número?)
            // Calcula distância para o primeiro e último ponto da rua
            const firstPoint = winnerStreet.items[0];
            const lastPoint = winnerStreet.items[winnerStreet.items.length - 1];
            
            const dFirst = Math.pow(firstPoint.lat - currentOrigin.lat, 2) + Math.pow(firstPoint.lng - currentOrigin.lng, 2);
            const dLast = Math.pow(lastPoint.lat - currentOrigin.lat, 2) + Math.pow(lastPoint.lng - currentOrigin.lng, 2);

            // Se estiver mais perto do final, inverte a rua
            if (dLast < dFirst) {
                winnerStreet.items.reverse();
                currentOrigin = firstPoint; // A nova origem será onde terminamos (que era o começo original)
            } else {
                currentOrigin = lastPoint; // Terminamos no fim normal
            }

            // Adiciona rua à rota
            finalRoute.push(...winnerStreet.items);
            
            // Remove dos pendentes
            pendingStreets = pendingStreets.filter(s => s.name !== winnerStreet.name);
            
            processedCount++;
            if (updateProgress) updateProgress(processedCount, totalStreets);

            // Delay API
            await new Promise(r => setTimeout(r, 400));

        } catch (e) {
            console.warn("Matrix Fail, fallback geo", e);
            const fallback = candidates[0];
            finalRoute.push(...fallback.items);
            currentOrigin = fallback.items[fallback.items.length - 1];
            pendingStreets = pendingStreets.filter(s => s.name !== fallback.name);
        }
    }
...
export default function App() {
  ...
  const optimizeRoute = async () => {
      ...
      try {
          ...
          // OTIMIZAÇÃO: STREET MATRIX
          const optimizedStops = await optimizeStreetMatrix(pending, pos, (curr, total) => {
              setOptimizeProgress(`Rua ${curr}/${total}`);
          });
          
          const updated = [...routes];
          updated[rIdx] = { ...updated[rIdx], stops: [...done, ...optimizedStops], optimized: true };
          setRoutes(updated);
          showToast("Otimização Rua-a-Rua Concluída!");
              ...
              {!searchQuery && !showMap && (
                  <div className="flex gap-3">
                      <button onClick={optimizeRoute} disabled={isOptimizing} className={`flex-1 py-3 rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition ${!activeRoute.optimized ? 'btn-highlight animate-pulse' : 'btn-secondary'}`}>
                          {isOptimizing ? (<div className="flex items-center gap-2"><Loader2 className="animate-spin" size={18}/><span>{optimizeProgress || 'Otimizando...'}</span></div>) : (<div className="flex items-center gap-2"><Sliders size={18}/> <span>Otimizar</span></div>)}
                      </button>
...
def main():
    print(f"--- Iniciando V26 (Street Matrix) {TIMESTAMP} ---")
    write_files()
    
    print("--- Git Push ---")
    subprocess.run("git add .", shell=True)
    subprocess.run(f'git commit -m "Update V26: Street-Level Matrix Optimization - {TIMESTAMP}"', shell=True)
