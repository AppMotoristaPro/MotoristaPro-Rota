...
CURRENT_BACKUP_PATH = os.path.join(BACKUP_DIR, f"refactor_fix_{TIMESTAMP}")

# Conteúdo dos arquivos
FILES_TO_WRITE = {
    ".env": """VITE_GOOGLE_MAPS_API_KEY=AIzaSyB8bI2MpTKfQHBTZxyPphB18TPlZ4b3ndU""",
...
def backup_files():
    print(f"--- Iniciando Backup para: {CURRENT_BACKUP_PATH} ---")
    ensure_dir(CURRENT_BACKUP_PATH)
    
    # Lista de arquivos para backup (os que vamos alterar)
    targets = [
        "src/App.jsx",
        ".gitignore",
        "package.json" 
    ...
    for target in targets:
        if os.path.exists(target):
            # Cria a estrutura de pastas no backup se necessário
            dest_path = os.path.join(CURRENT_BACKUP_PATH, target)
            ...
            print(f"[Backup Skip] {target} não existe ainda.")

def write_files():
    print("\n--- Escrevendo Novos Arquivos ---")
    for filepath, content in FILES_TO_WRITE.items():
        ...
        with open(filepath, "w", encoding="utf-8") as f:
            f.write(content)
        print(f"[Criado/Atualizado] {filepath}")
...
def git_operations():
    ...
    try:
        ...
        print("Commitando...")
        subprocess.run(["git", "commit", "-m", f"Refactor: Componentes e Segurança {TIMESTAMP}"], check=False)
        
        print("Tentando Push (pode falhar se remoto não configurado)...")
        # Tenta push, mas não trava o script se falhar (ex: sem internet ou sem remote)
...
def self_destruct():
    ...
    try:
        os.remove(__file__)
        print(f"Script {__file__} deletado com sucesso.")
    except Exception as e:
...
def main():
    try:
        ...
        write_files()
        git_operations()
        print("\n\nSUCESSO! Projeto refatorado.")
        print("1. API Key movida para .env")
        print("2. Componentes criados em src/components")
        print("3. App.jsx limpo e organizado")
