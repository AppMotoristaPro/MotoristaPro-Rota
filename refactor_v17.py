...
CURRENT_BACKUP_PATH = os.path.join(BACKUP_DIR, f"update_v17_{TIMESTAMP}")

# CHAVE API
API_KEY_VALUE = "AIzaSyB8bI2MpTKfQHBTZxyPphB18TPlZ4b3ndU"

# --- NOVO ALGORITMO INTELIGENTE PARA O APP.JSX ---
...
const DB_KEY = 'mp_db_v56_dynamic_direction';
...
// --- ALGORITMO V17: OTIMIZAÇÃO DINÂMICA DE EXTREMIDADES ---
const optimizeDynamicStreets = async (allStops, startPos) => {
    
    // 1. Agrupamento por Rua
    const streetMap = {};
    ...
    // 2. Preparar Objetos de Rua com Extremidades
    const streetsPool = [];
    Object.keys(streetMap).forEach(key => {
        let items = streetMap[key];
        
        // Ordena numericamente (base padrão)
        items.sort((a, b) => extractHouseNumber(a.address) - extractHouseNumber(b.address));
        
        // Define pontos de entrada (Min) e saída (Max) naturais
        const minPoint = items[0]; // Menor número
        const maxPoint = items[items.length - 1]; // Maior número

        streetsPool.push({
            id: key,
            items: items,
            startCoord: { lat: minPoint.lat, lng: minPoint.lng }, // Coordenada do nº menor
            endCoord: { lat: maxPoint.lat, lng: maxPoint.lng }    // Coordenada do nº maior
        });
    });

    // 3. Ordenação Global com Decisão de Sentido (Snake Sort)
    let sortedStreets = [];
    let currentPointer = startPos;
    let pool = [...streetsPool];

    while (pool.length > 0) {
        let bestIdx = -1;
        let bestDirection = 'asc'; // 'asc' (Min->Max) ou 'desc' (Max->Min)
        let minGlobalDist = Infinity;
        
        for(let i=0; i<pool.length; i++) {
            const street = pool[i];
            
            // Distância para o MENOR número (Entrada convencional)
            const distToStart = Math.pow(street.startCoord.lat - currentPointer.lat, 2) + 
                                Math.pow(street.startCoord.lng - currentPointer.lng, 2);
            
            // Distância para o MAIOR número (Entrada invertida)
            const distToEnd = Math.pow(street.endCoord.lat - currentPointer.lat, 2) + 
                              Math.pow(street.endCoord.lng - currentPointer.lng, 2);

            // Verifica qual ponta está mais perto
            if (distToStart < minGlobalDist) {
                minGlobalDist = distToStart;
                bestIdx = i;
                bestDirection = 'asc';
            }
            if (distToEnd < minGlobalDist) {
                minGlobalDist = distToEnd;
                bestIdx = i;
                bestDirection = 'desc';
            }
        }
        
        const chosenStreet = pool[bestIdx];
        
        // Aplica a ordenação baseada na decisão
        if (bestDirection === 'desc') {
            chosenStreet.items.reverse(); // Inverte para fazer do maior para o menor
            currentPointer = chosenStreet.startCoord; // A nova posição do motorista será no menor número (fim da rua invertida)
        } else {
            // Já está 'asc' (menor pro maior)
            currentPointer = chosenStreet.endCoord; // A nova posição será no maior número
        }

        sortedStreets.push(chosenStreet);
    ...
    // 5. Refino Google (Directions API) com Sobreposição
    // Mantemos isso para corrigir nuances de trânsito (ex: rua sem saída ou mão única que força a ordem)
    ...
    while (unvisited.length > 0) {
        ...
        try {
            ...
            const leftovers = optimizedBatch.slice(KEEP_SIZE);
...
export default function App() {
  ...
  const optimizeRoute = async () => {
      ...
      try {
          // NOVA OTIMIZAÇÃO: EXTREMIDADES DINÂMICAS
          const optimizedStops = await optimizeDynamicStreets(pending, pos);
          
          const updated = [...routes];
          updated[rIdx] = { ...updated[rIdx], stops: [...done, ...optimizedStops], optimized: true };
          setRoutes(updated);
          showToast("Otimização Inteligente Concluída!");
...
def main():
    print(f"--- Iniciando V17 (Otimização Dinâmica) {TIMESTAMP} ---")
    write_files()
    
    print("--- Git Push ---")
    subprocess.run("git add .", shell=True)
    subprocess.run(f'git commit -m "Update V17: Otimização com Sentido de Rua Dinâmico - {TIMESTAMP}"', shell=True)
