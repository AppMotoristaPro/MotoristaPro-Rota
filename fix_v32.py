...
CURRENT_BACKUP_PATH = os.path.join(BACKUP_DIR, f"update_v32_{TIMESTAMP}")

# CHAVE API
API_KEY_VALUE = "AIzaSyB8bI2MpTKfQHBTZxyPphB18TPlZ4b3ndU"

# --- 1. MAP VIEW (CORREÇÃO OBJECT OBJECT) ---
...
# --- 2. APP (HEADER NOVO, VALOR E LOGICA VOLTAR) ---
...
export default function App() {
  ...
  const [newRouteValue, setNewRouteValue] = useState(''); // ITEM 5: Valor da rota
  ...
  const getCurrentLocation = async (silent = false) => {
      try {
          // Se for silent, tenta pegar sem pedir permissão explícita (se já tiver)
  ...
  // ITEM 1: Permissão inteligente
  const ensurePermissionAndPos = async () => {
      if (userPos) return userPos;
      
      // Se não tem pos, pede permissão explicita
  ...
  const createRoute = () => {
      ...
      setRoutes([{ 
          ...
          date: newRouteDate, 
          value: newRouteValue, // ITEM 5: Salva valor
          stops: tempStops, 
          optimized: true 
      }, ...routes]);
  ...
  const startReorderMode = () => {
      handleToggleMap(); // Garante que mapa abre
  ...
  // ITEM 3: Botão Voltar Lógico
  const handleBack = () => {
      if (showMap) {
          setShowMap(false);
          setIsReordering(false); // Garante sair do modo edição
      } else {
          setView('home');
      }
  };
  ...
  return (
      <div className="flex flex-col h-screen bg-slate-50 relative">
          ...
          {/* ITEM 4: HEADER DA ROTA (Dashboard style) */}
          <div className="bg-white p-4 shadow-sm z-20 sticky top-0 rounded-b-3xl">
              <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="text-right">
                      ...
                      <span className="text-[10px] text-slate-400">{calculateProgress(activeRoute.stops)}%</span>
                  </div>
              </div>
...
def main():
    print(f"--- Iniciando V32 (Financeiro + UX) {TIMESTAMP} ---")
    ...
    subprocess.run(f'git commit -m "Update V32: Finance Fields + Map UX Fixes - {TIMESTAMP}"', shell=True)
