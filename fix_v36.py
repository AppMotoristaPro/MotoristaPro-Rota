...
export default function App() {
  ...
  // CORREÇÃO 1: Inicializar com valores seguros
      ...
      if (isLoaded && nextGroup && userPos) {
          ...
          service.route({
              ...
              if (status === 'OK') {
                  ...
                  // CORREÇÃO 2: Atualiza Métricas com Segurança
          ...
          // Reseta métricas se não tiver rota
      ...
      <div className="min-h-screen bg-slate-50 flex flex-col">
          ...
          <div className="flex-1 p-6 space-y-4 overflow-y-auto">
              ...
              <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100">
                      <div className="flex items-center gap-2 text-slate-400 mb-1"><Check size={16}/><span className="text-xs font-bold uppercase">Sucesso</span></div>
                      <div className="text-2xl font-bold text-green-600">{dashboardStats.successRate}%</div>
                  </div>
                  <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100">
                      <div className="flex items-center gap-2 text-slate-400 mb-1"><Building size={16}/><span className="text-xs font-bold uppercase">Média / Rota</span></div>
      ...
      <div className="min-h-screen pb-24 px-6 pt-12 bg-slate-100">
          <div className="flex justify-between items-center mb-8">
              ...
              <div className="flex gap-2">
                  <button onClick={() => setView('dashboard')} className="bg-white p-3 rounded-2xl shadow-sm text-slate-600 hover:text-blue-600 transition"><Info size={24}/></button>
      ...
      <div className="min-h-screen bg-slate-50 flex flex-col">
          ...
          <div className="flex-1 p-6 space-y-6 overflow-y-auto">
              <div className="space-y-4">
                  ...
                  {/* CORREÇÃO 2: Layout ajustado para não esticar */}
                  <div className="flex flex-row gap-4">
                      <div className="w-1/2">
                          ...
                          <div className="flex items-center bg-white p-4 rounded-xl border border-slate-200 focus-within:border-blue-500 transition">
                              <Calendar className="text-slate-300 mr-2 shrink-0" size={20}/>
                              <input type="date" className="w-full outline-none text-sm font-medium bg-transparent" value={newRouteDate} onChange={e => setNewRouteDate(e.target.value)}/>
                          </div>
                      </div>
                      <div className="w-1/2">
                          <label className="text-xs font-bold text-slate-500 uppercase ml-1 mb-1 block">Valor (R$)</label>
                          <div className="flex items-center bg-white p-4 rounded-xl border border-slate-200 focus-within:border-blue-500 transition">
                              <DollarSign className="text-slate-300 mr-1 shrink-0" size={20}/>
                              <input type="number" className="w-full outline-none text-sm font-medium bg-transparent" placeholder="0,00" value={newRouteValue} onChange={e => setNewRouteValue(e.target.value)}/>
                          </div>
                      </div>
                  </div>
              </div>
